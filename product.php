...
// $row = mysqli_fetch_assoc($catpost_exc);
...
if (isset($_POST['save'])) {
  ...
  $image = mysqli_real_escape_string($conn,$_FILES['image']['name']);
    ...
    if (move_uploaded_file($imgTmp, $imgFolder)) {
        ...
        $error = "Uploaded failed".mysqli_error($conn);
        $msg = "<div class='alert alert-danger'>$error</div>";
    }
    ...
    $insert_product = "INSERT INTO product (image, header, title, content, footer) VALUES ('$image', '$header', '$title', '$content', '$footer') ";
    $insert_product_exc = mysqli_query($conn, $insert_product);

    if ($insert_product_exc) {
        $error = "Category Added successfully".mysqli_error($conn);
        $msg = "<div class='alert alert-danger'>$error</div>";
    }else {
    ...
    // // UPDATE PER ROW
    // $update_student = "UPDATE category SET image ='$image', title ='$title' WHERE id ='$id' ";
...
if (isset($_POST['edit'])) {
    ...
    $edit_product = "INSERT INTO product (image, header, title, content, footer) VALUES ('$image', '$header', '$title', '$content', '$footer') ";
    $edit_product_exc = mysqli_query($conn, $edit_product);

    if ($edit_product_exc) {
        $error = "Product Added successfully".mysqli_error($conn);
        $msg = "<div class='alert alert-danger'>$error</div>";
    }else {
        $error = "Product not Added".mysqli_error($conn);
        $msg = "<div class='alert alert-danger'>$error</div>";
    }
...
<div class="navbar bg-primary"></div>
    ...
    <div class="row justify-content-around bg-primary">
        ...
        <div class="sidebar col-2 d-block navbar bg-primary fw-bold my-3">
            <div class="col-12">
                <h3 class="text-warning fw-bold mb-4">CozaStore <br> <span class=" text-black">Product</span></h3>
                ...
                <a href="#" class="my-2 nav-link strong text-white">Prodoct</a><br>
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="col-md-1"> <?= $row['image'] ?> </td>
                                  <td class="col-md-1"> <?= $row['header'] ?> </td>
                                <td class="col-md-1"> <?= $row['title'] ?> </td>
                                  <td class="col-md-1"> <?= $row['content'] ?> </td>
                                  <td class="col-md-1"> <?= $row['footer'] ?> </td>
                                ...
                                <td class="text-center d-flex justify-content-center align-items-center">
                                    ...
                                    <a href="product.php?delete_id= <?= $row['id'] ?>" class="btn btn-sm btn-outline-danger">Delete</a>
                                </td>
                            </tr>
                                        ...
                                        <form action="product.php" method="POST" >
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                  ...
                                                  <div class="mb-3">
                                                    <label class="form-label">Upload Image</label>
                                                    <input type="file" name="image" class="form-control" value="<?= $row['image'] ?>" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label">Header</label>
                                                    <input type="text" name="header" class="form-control"
                                                    value="<?= $row['header'] ?>" required>
                                                </div>
                                                  <div class="mb-3">
                                                    <label class="form-label">Title</label>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    value="<?= $row['content'] ?>" required>
                                                </div>
                                                  <div class="mb-3">
                                                    <label class="form-label">Footer</label>
                                                    <input type="text" name="footer" class="form-control"
                                                    value="<?= $row['footer'] ?>" required>
                                                </div>
                                                  <!-- <div class="mb-3">
                                                    <label class="form-label">Description</label>
